/*
** $Id: vmkc.c $
** Vmk compiler (saves bytecodes to files; also lists bytecodes)
...
#define vmkU_print	PrintFunction

#define PROGNAME	"vmkc"		/* default program name */
...
static const Proto* combine(vmk_State* L, int n)
{
 if (n==1)
  return toproto(L,-1);
 else
 {
  Proto* f;
  int i=n;
  if (vmk_load(L,reader,&i,"=(" PROGNAME ")",NULL)!=VMK_OK) fatal(vmk_tostring(L,-1));
  f=toproto(L,-1);
  for (i=0; i<n; i++)
  {
...
static int writer(vmk_State* L, const void* p, size_t size, void* u)
{
 UNUSED(L);
...
static int pmain(vmk_State* L)
{
 ...
 int i;
 tmname=G(L)->tmname;
 if (!vmk_checkstack(L,argc)) fatal("too many input files");
 for (i=0; i<argc; i++)
 {
  const char* filename=IS("-") ? NULL : argv[i];
  if (vmkL_loadfile(L,filename)!=VMK_OK) fatal(vmk_tostring(L,-1));
 }
 f=combine(L,argc);
 if (listing) vmkU_print(f,listing>1);
 if (dumping)
 {
  FILE* D= (output==NULL) ? stdout : fopen(output,"wb");
  if (D==NULL) cannot("open");
  vmk_lock(L);
  vmkU_dump(L,f,writer,D,stripping);
  vmk_unlock(L);
...
int main(int argc, char* argv[])
{
 ...
 vmk_pushcfunction(L,&pmain);
 vmk_pushinteger(L,argc);
 vmk_pushlightuserdata(L,argv);
 if (vmk_pcall(L,2,0,0)!=VMK_OK) fatal(vmk_tostring(L,-1));
 vmk_close(L);
 return EXIT_SUCCESS;
}
...
static void PrintConstant(const Proto* f, int i)
{
 ...
 switch (ttypetag(o))
 {
  ...
  case VMK_VNUMFLT:
	{
	char buff[100];
	sprintf(buff,VMK_NUMBER_FMT,fltvalue(o));
	printf("%s",buff);
	if (buff[strspn(buff,"-0123456789")]=='\0') printf(".0");
	break;
	}
  case VMK_VNUMINT:
	printf(VMK_INTEGER_FMT,ivalue(o));
	break;
  case VMK_VSHRSTR:
  case VMK_VLNGSTR:
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  int isk=GETARG_k(i);
  int line=vmkG_getfuncline(f,pc);
  printf("\t%d\t",pc+1);
