...
const TValue *vmkT_gettm (Table *events, TMS event, TString *ename) {
  const TValue *tm = vmkH_getshortstr(events, ename);
...
const TValue *vmkT_gettmbyobj (vmk_State *L, const TValue *o, TMS event) {
  ...
  return (mt ? vmkH_getshortstr(mt, G(L)->tmname[event]) : &G(L)->nilvalue);
}
...
const char *vmkT_objtypename (vmk_State *L, const TValue *o) {
  ...
  if ((ttistable(o) && (mt = hvalue(o)->metatable) != NULL) ||
    ...
    const TValue *name = vmkH_getshortstr(mt, vmkS_new(L, "__name"));
...
void vmkT_callTMres (vmk_State *L, const TValue *f, const TValue *p1,
...
void vmkT_tryconcatTM (vmk_State *L) {
  ...
  if (l_unlikely(!callbinTM(L, s2v(top - 2), s2v(top - 1), top - 2,
                               TM_CONCAT)))
    vmkG_concaterror(L, s2v(top - 2), s2v(top - 1));
}
