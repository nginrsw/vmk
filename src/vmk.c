...
#define vmk_readline(L,b,p)	((void)L, ((b)=readline(p)) != NULL)
#define vmk_saveline(L,line)	((void)L, add_history(line))
#define vmk_freeline(L,b)	((void)L, free(b))

#else				/* }{ */

#define vmk_initreadline(L)  ((void)L)
#define vmk_readline(L,b,p) \
        ((void)L, fputs(p, stdout), fflush(stdout),  /* show prompt */ \
        fgets(b, VMK_MAXINPUT, stdin) != NULL)  /* get line */
#define vmk_saveline(L,line)	{ (void)L; (void)line; }
#define vmk_freeline(L,b)	{ (void)L; (void)b; }
...
static int pushline (vmk_State *L, int firstline) {
  ...
  int readstatus = vmk_readline(L, b, prmt);
  if (readstatus == 0)
  ...
  if (firstline && b[0] == '=')  /* for compatibility with 5.2, ... */
    vmk_pushfstring(L, "return %s", b + 1);  /* change '=' to 'return' */
  else
...
static int addreturn (vmk_State *L) {
  ...
  if (status == VMK_OK) {
    ...
    if (line[0] != '\0')  /* non empty? */
      vmk_saveline(L, line);  /* keep history */
  }
  else
...
/*
** Read multiple lines until a complete Vmk statement
*/
static int multiline (vmk_State *L) {
  for (;;) {  /* repeat until gets a complete statement */
    size_t len;
    const char *line = vmk_tolstring(L, 1, &len);  /* get what it has */
...
static int pmain (vmk_State *L) {
  ...
  vmkL_openlibs(L);  /* open standard libraries */
  createargtable(L, argv, argc, script);  /* create table 'arg' */
  vmk_gc(L, VMK_GCRESTART);  /* start GC... */
  vmk_gc(L, VMK_GCGEN, 0, 0);  /* ...in generational mode */
