...
VMKLIB_API int vmkL_argerror (vmk_State *L, int arg, const char *extramsg) {
  ...
  return vmkL_error(L, "bad argument #%d to '%s' (%s)",
...
static size_t newbuffsize (vmkL_Buffer *B, size_t sz) {
  ...
  if (l_unlikely(MAX_SIZET - sz < B->n))  /* overflow in (B->n + sz)? */
    return vmkL_error(B->L, "buffer too large");
...
#define freelist	(VMK_RIDX_LAST + 1)
...
VMKLIB_API int vmkL_ref (vmk_State *L, int t) {
  ...
  t = vmk_absindex(L, t);
  if (vmk_rawgeti(L, t, freelist) == VMK_TNIL) {  /* first access? */
    ref = 0;  /* list is empty */
    vmk_pushinteger(L, 0);  /* initialize as an empty list */
    vmk_rawseti(L, t, freelist);  /* ref = t[freelist] = 0 */
  }
  else {  /* already initialized */
    vmk_assert(vmk_isinteger(L, -1));
    ref = (int)vmk_tointeger(L, -1);  /* ref = t[freelist] */
  }
  vmk_pop(L, 1);  /* remove element from stack */
  if (ref != 0) {  /* any free element? */
    vmk_rawgeti(L, t, ref);  /* remove it from list */
    vmk_rawseti(L, t, freelist);  /* (t[freelist] = t[ref]) */
  }
...
VMKLIB_API void vmkL_unref (vmk_State *L, int t, int ref) {
  if (ref >= 0) {
    t = vmk_absindex(L, t);
    vmk_rawgeti(L, t, freelist);
    vmk_assert(vmk_isinteger(L, -1));
    vmk_rawseti(L, t, ref);  /* t[ref] = t[freelist] */
    vmk_pushinteger(L, ref);
    vmk_rawseti(L, t, freelist);  /* t[freelist] = ref */
  }
}
...
VMKLIB_API const char *vmkL_tolstring (vmk_State *L, int idx, size_t *len) {
  ...
  if (vmkL_callmeta(L, idx, "__tostring")) {  /* metafield? */
    ...
    switch (vmk_type(L, idx)) {
      case VMK_TNUMBER: {
        if (vmk_isinteger(L, idx))
          vmk_pushfstring(L, "%I", (VMKI_UACINT)vmk_tointeger(L, idx));
        else
          vmk_pushfstring(L, "%f", (VMKI_UACNUMBER)vmk_tonumber(L, idx));
        break;
      }
      case VMK_TSTRING:
        vmk_pushvalue(L, idx);
...
VMKLIB_API void vmkL_addgsub (vmkL_Buffer *b, const char *s,
  ...
  while ((wild = strstr(s, p)) != NULL) {
    vmkL_addlstring(b, s, wild - s);  /* push prefix */
...
VMKLIB_API vmk_State *vmkL_newstate (void) {
  vmk_State *L = vmk_newstate(l_alloc, NULL);
  if (l_likely(L)) {
    vmk_atpanic(L, &panic);
