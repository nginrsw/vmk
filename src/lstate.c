...
static unsigned int vmki_makeseed (vmk_State *L) {
...
VMK_API int vmk_setcstacklimit (vmk_State *L, unsigned int limit) {
  UNUSED(L); UNUSED(limit);
  return VMKI_MAXCCALLS;  /* warning?? */
}
...
static void freestack (vmk_State *L) {
  ...
  freeCI(L);
  vmk_assert(L->nci == 0);
  vmkM_freearray(L, L->stack.p, stacksize(L) + EXTRA_STACK);  /* free stack */
}
...
static void init_registry (vmk_State *L, global_State *g) {
  ...
  setthvalue(L, &registry->array[VMK_RIDX_MAINTHREAD - 1], L);
  /* registry[VMK_RIDX_GLOBALS] = new table (table of globals) */
  sethvalue(L, &registry->array[VMK_RIDX_GLOBALS - 1], vmkH_new(L));
}
...
static void close_state (vmk_State *L) {
  ...
  vmkM_freearray(L, G(L)->strt.hash, G(L)->strt.size);
  freestack(L);
...
/*
** Deprecated! Use 'vmk_closethread' instead.
*/
VMK_API int vmk_resetthread (vmk_State *L) {
  return vmk_closethread(L, NULL);
}


VMK_API vmk_State *vmk_newstate (vmk_Alloc f, void *ud) {
  int i;
  vmk_State *L;
  global_State *g;
  ...
  setgcparam(g->gcpause, VMKI_GCPAUSE);
  setgcparam(g->gcstepmul, VMKI_GCMUL);
  g->gcstepsize = VMKI_GCSTEPSIZE;
  setgcparam(g->genmajormul, VMKI_GENMAJORMUL);
  g->genminormul = VMKI_GENMINORMUL;
  for (i=0; i < VMK_NUMTAGS; i++) g->mt[i] = NULL;
