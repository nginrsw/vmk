...
static int l_strton (const TValue *obj, TValue *result) {
  ...
  if (!cvt2num(obj))  /* is object not a string? */
    ...
    return (vmkO_str2num(getstr(st), result) == tsslen(st) + 1);
  }
}
...
static int floatforloop (StkId ra) {
  vmk_Number step = fltvalue(s2v(ra + 2));
  vmk_Number limit = fltvalue(s2v(ra + 1));
  vmk_Number idx = fltvalue(s2v(ra));  /* internal index */
...
*/
void vmkV_finishget (vmk_State *L, const TValue *t, TValue *key, StkId val,
                      const TValue *slot) {
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (vmkV_fastget(L, t, key, slot, vmkH_get)) {  /* fast track? */
...
** 'vmkV_fastget' would have done the job.)
*/
void vmkV_finishset (vmk_State *L, const TValue *t, TValue *key,
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (slot != NULL) {  /* is 't' a table? */
      ...
      vmk_assert(isempty(slot));  /* slot must be empty */
      tm = fasttm(L, h->metatable, TM_NEWINDEX);  /* get metamethod */
      if (tm == NULL) {  /* no metamethod? */
        vmkH_finishset(L, h, key, slot, val);  /* set new value */
        invalidateTMcache(h);
    ...
    if (vmkV_fastget(L, t, key, slot, vmkH_get)) {
      vmkV_finishfastset(L, t, slot, val);
      return;  /* done */
    }
...
int vmkV_equalobj (vmk_State *L, const TValue *t1, const TValue *t2) {
  ...
  if (tm == NULL)  /* no TM? */
    ...
    vmkT_callTMres(L, tm, t1, t2, L->top.p);  /* call TM */
...
void vmkV_objlen (vmk_State *L, StkId ra, const TValue *rb) {
  ...
  switch (ttypetag(rb)) {
    case VMK_VTABLE: {
      ...
      setivalue(s2v(ra), vmkH_getn(h));  /* else primitive len */
      return;
    }
    case VMK_VSHRSTR: {
  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_GETTABUP) {
        ...
        if (vmkV_fastget(L, upval, key, slot, vmkH_getshortstr)) {
          setobj2s(L, ra, slot);
        }
        else
          Protect(vmkV_finishget(L, upval, rc, ra, slot));
        vmbreak;
      }
      vmcase(OP_GETTABLE) {
        StkId ra = RA(i);
        ...
        if (ttisinteger(rc)  /* fast track for integers? */
            ? (cast_void(n = ivalue(rc)), vmkV_fastgeti(L, rb, n, slot))
            : vmkV_fastget(L, rb, rc, slot, vmkH_get)) {
          setobj2s(L, ra, slot);
        }
        else
          Protect(vmkV_finishget(L, rb, rc, ra, slot));
        vmbreak;
      }
      vmcase(OP_GETI) {
        StkId ra = RA(i);
        ...
        TValue *rb = vRB(i);
        int c = GETARG_C(i);
        if (vmkV_fastgeti(L, rb, c, slot)) {
          setobj2s(L, ra, slot);
        }
        else {
          TValue key;
          setivalue(&key, c);
          Protect(vmkV_finishget(L, rb, &key, ra, slot));
        }
        vmbreak;
      }
      vmcase(OP_GETFIELD) {
        ...
        TValue *rc = KC(i);
        TString *key = tsvalue(rc);  /* key must be a short string */
        if (vmkV_fastget(L, rb, key, slot, vmkH_getshortstr)) {
          setobj2s(L, ra, slot);
        }
        else
          Protect(vmkV_finishget(L, rb, rc, ra, slot));
      ...
      vmcase(OP_SETTABUP) {
        ...
        if (vmkV_fastget(L, upval, key, slot, vmkH_getshortstr)) {
          vmkV_finishfastset(L, upval, slot, rc);
        }
        else
          Protect(vmkV_finishset(L, upval, rb, rc, slot));
        vmbreak;
      }
      vmcase(OP_SETTABLE) {
        StkId ra = RA(i);
        ...
        if (ttisinteger(rb)  /* fast track for integers? */
            ? (cast_void(n = ivalue(rb)), vmkV_fastgeti(L, s2v(ra), n, slot))
            : vmkV_fastget(L, s2v(ra), rb, slot, vmkH_get)) {
          vmkV_finishfastset(L, s2v(ra), slot, rc);
        }
        else
          Protect(vmkV_finishset(L, s2v(ra), rb, rc, slot));
        vmbreak;
      }
      vmcase(OP_SETI) {
        StkId ra = RA(i);
        ...
        int c = GETARG_B(i);
        TValue *rc = RKC(i);
        if (vmkV_fastgeti(L, s2v(ra), c, slot)) {
          vmkV_finishfastset(L, s2v(ra), slot, rc);
        }
        else {
          TValue key;
          setivalue(&key, c);
          Protect(vmkV_finishset(L, s2v(ra), &key, rc, slot));
        }
        vmbreak;
      }
      vmcase(OP_SETFIELD) {
        ...
        TValue *rc = RKC(i);
        TString *key = tsvalue(rb);  /* key must be a short string */
        if (vmkV_fastget(L, s2v(ra), key, slot, vmkH_getshortstr)) {
          vmkV_finishfastset(L, s2v(ra), slot, rc);
        }
        else
          Protect(vmkV_finishset(L, s2v(ra), rb, rc, slot));
      ...
      vmcase(OP_NEWTABLE) {
        ...
        vmk_assert((!TESTARG_k(i)) == (GETARG_Ax(*pc) == 0));
      ...
      vmcase(OP_SELF) {
        ...
        setobj2s(L, ra + 1, rb);
        if (vmkV_fastget(L, rb, key, slot, vmkH_getstr)) {
          setobj2s(L, ra, slot);
        }
        else
          Protect(vmkV_finishget(L, rb, rc, ra, slot));
      ...
      vmcase(OP_FORLOOP) {
        ...
        if (ttisinteger(s2v(ra + 2))) {  /* integer loop? */
          vmk_Unsigned count = l_castS2U(ivalue(s2v(ra + 1)));
          if (count > 0) {  /* still more iterations? */
            vmk_Integer step = ivalue(s2v(ra + 2));
            vmk_Integer idx = ivalue(s2v(ra));  /* internal index */
      ...
      vmcase(OP_TFORPREP) {
       StkId ra = RA(i);
        ...
        halfProtect(vmkF_newtbcupval(L, ra + 3));
        pc += GETARG_Bx(i);
      ...
      vmcase(OP_SETLIST) {
        ...
        if (last > vmkH_realasize(h))  /* needs more space? */
