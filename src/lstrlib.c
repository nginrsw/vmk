...
** 'size_t'. (We assume that 'vmk_Integer' cannot be smaller than 'int'.)
*/
...
static int str_byte (vmk_State *L) {
  ...
  for (i=0; i<n; i++)
    vmk_pushinteger(L, uchar(s[posi+i-1]));
  return n;
}
  ...
  if (i >= ms->level) {
    ...
    *cap = ms->capture[i].init;
    if (l_unlikely(capl == CAP_UNFINISHED))
      ...
      vmk_pushinteger(ms->L, (ms->capture[i].init - ms->src_init) + 1);
    return capl;
  }
}
...
static int str_find_aux (vmk_State *L, int find) {
  ...
  if (find && (vmk_toboolean(L, 4) || nospecials(p, lp))) {
    ...
    if (s2) {
      vmk_pushinteger(L, (s2 - s) + 1);
      vmk_pushinteger(L, (s2 - s) + lp);
      return 2;
    }
  }
  else {
    ...
    do {
      ...
      if ((res=match(&ms, s1, p)) != NULL) {
        if (find) {
          vmk_pushinteger(L, (s1 - s) + 1);  /* start */
          vmk_pushinteger(L, res - s);   /* end */
...
static void add_s (MatchState *ms, vmkL_Buffer *b, const char *s,
  ...
  while ((p = (char *)memchr(news, L_ESC, l)) != NULL) {
    vmkL_addlstring(b, news, p - news);
    p++;  /* skip ESC */
    if (*p == L_ESC)  /* '%%' */
      vmkL_addchar(b, *p);
    else if (*p == '0')  /* '%0' */
        vmkL_addlstring(b, s, e - s);
...
static int add_value (MatchState *ms, vmkL_Buffer *b, const char *s,
  ...
  if (!vmk_toboolean(L, -1)) {  /* nil or false? */
    ...
    vmkL_addlstring(b, s, e - s);  /* keep original text */
    return 0;  /* no changes */
  }
...
static int str_gsub (vmk_State *L) {
  ...
  vmk_Integer max_s = vmkL_optinteger(L, 4, srcl + 1);  /* max replacements */
  int anchor = (*p == '^');
  ...
  if (!changed)  /* no changes? */
    ...
    vmkL_addlstring(&b, src, ms.src_end-src);
...
*/
static vmk_Number adddigit (char *buff, int n, vmk_Number x) {
...
static int num2straux (char *buff, int sz, vmk_Number x) {
...
static int vmk_number2strx (vmk_State *L, char *buff, int sz,
...
static int getnumlimit (Header *h, const char **fmt, int df) {
  ...
  if (l_unlikely(sz > MAXINTSIZE || sz <= 0))
    return vmkL_error(h->L, "integral size (%d) out of limits [1,%d]",
                            sz, MAXINTSIZE);
  return sz;
}
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      case Kint: {  /* signed integers */
        ...
        packint(&b, (vmk_Unsigned)n, h.islittle, size, (n < 0));
        break;
      }
      case Kuint: {  /* unsigned integers */
        ...
        packint(&b, (vmk_Unsigned)n, h.islittle, size, 0);
        break;
      }
      case Kfloat: {  /* C float */
      ...
      case Kchar: {  /* fixed-size string */
        ...
        vmkL_argcheck(L, len <= (size_t)size, arg,
                         "string longer than given size");
        vmkL_addlstring(&b, s, len);  /* add string */
        while (len++ < (size_t)size)  /* pad extra space */
          vmkL_addchar(&b, VMKL_PACKPADBYTE);
        break;
      }
      case Kstring: {  /* strings with length count */
        size_t len;
        const char *s = vmkL_checklstring(L, arg, &len);
        vmkL_argcheck(L, size >= (int)sizeof(size_t) ||
                         len < ((size_t)1 << (size * NB)),
                         arg, "string length does not fit in given size");
        packint(&b, (vmk_Unsigned)len, h.islittle, size, 0);  /* pack length */
...
static int str_packsize (vmk_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    vmkL_argcheck(L, totalsize <= MAXSIZE - size, 1,
                     "format result too large");
    totalsize += size;
  }
  vmk_pushinteger(L, (vmk_Integer)totalsize);
  return 1;
}
...
static int str_unpack (vmk_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    vmkL_argcheck(L, (size_t)ntoalign + size <= ld - pos, 2,
    ...
    switch (opt) {
      case Kint:
      case Kuint: {
        vmk_Integer res = unpackint(L, data + pos, h.islittle, size,
                                       (opt == Kint));
  ...
  vmk_pushinteger(L, pos + 1);  /* next position */
  return n + 1;
}
