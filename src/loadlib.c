...
*/
#if defined(__GNUC__)
#define cast_func(p) (__extension__ (vmk_CFunction)(p))
#else
#define cast_func(p) ((vmk_CFunction)(p))
#endif
...
static vmk_CFunction lsys_sym (vmk_State *L, void *lib, const char *sym) {
  vmk_CFunction f = cast_func(dlsym(lib, sym));
...
static vmk_CFunction lsys_sym (vmk_State *L, void *lib, const char *sym) {
  vmk_CFunction f = (vmk_CFunction)(voidf)GetProcAddress((HMODULE)lib, sym);
...
static void setpath (vmk_State *L, const char *fieldname,
  ...
  if (path == NULL || noenv(L))  /* no environment variable? */
    vmk_pushstring(L, dft);  /* use default */
    ...
    if (path < dftmark) {  /* is there a prefix before ';;'? */
      vmkL_addlstring(&b, path, dftmark - path);  /* add it */
      vmkL_addchar(&b, *VMK_PATH_SEP);
    }
    vmkL_addstring(&b, dft);  /* add default */
    if (dftmark < path + len - 2) {  /* is there a suffix after ';;'? */
      vmkL_addchar(&b, *VMK_PATH_SEP);
      vmkL_addlstring(&b, dftmark + 2, (path + len - 2) - dftmark);
    }
    vmkL_pushresult(&b);
  }
  setprogdir(L);
...
static int loadfunc (vmk_State *L, const char *filename, const char *modname) {
  ...
  if (mark) {
    int stat;
    openfunc = vmk_pushlstring(L, modname, mark - modname);
...
static void findloader (vmk_State *L, const char *name) {
  ...
  for (i = 1; ; i++) {
    ...
    if (l_unlikely(vmk_rawgeti(L, 3, i) == VMK_TNIL)) {  /* no more searchers? */
      ...
      vmkL_buffsub(&msg, 2);  /* remove prefix */
