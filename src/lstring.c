...
*/
#define MAXSTRTB	cast_int(vmkM_limitN(MAX_INT, TString*))
...
unsigned int vmkS_hash (const char *str, size_t l, unsigned int seed) {
...
unsigned int vmkS_hashlongstr (TString *ts) {
...
*/
static TString *createstrobj (vmk_State *L, size_t l, int tag, unsigned int h) {
  TString *ts;
  GCObject *o;
...
TString *vmkS_createlngstrobj (vmk_State *L, size_t l) {
  TString *ts = createstrobj(L, l, VMK_VLNGSTR, G(L)->seed);
  ts->u.lnglen = l;
...
static TString *internshrstr (vmk_State *L, const char *str, size_t l) {
  ...
  ts = createstrobj(L, l, VMK_VSHRSTR, h);
...
Udata *vmkS_newudata (vmk_State *L, size_t s, int nuvalue) {
  Udata *u;
  int i;
  GCObject *o;
