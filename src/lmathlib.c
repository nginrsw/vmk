...
static int math_ceil (vmk_State *L) {
  if (vmk_isinteger(L, 1))
    vmk_settop(L, 1);  /* integer is its own ceil */
  else {
...
static int math_random (vmk_State *L) {
  ...
  switch (vmk_gettop(L)) {  /* check number of arguments */
    ...
    case 1: {  /* only upper limit */
      ...
      if (up == 0) {  /* single 0 as argument? */
        vmk_pushinteger(L, I2UInt(rv));  /* full random integer */
        return 1;
      }
      break;
    }
  ...
  vmk_pushinteger(L, p + (vmk_Unsigned)low);
  return 1;
}
...
static void setseed (vmk_State *L, Rand64 *state,
  ...
  vmk_pushinteger(L, n1);
  vmk_pushinteger(L, n2);
}
...
** randomization).
*/
static void randseed (vmk_State *L, RanState *state) {
  vmk_Unsigned seed1 = (vmk_Unsigned)time(NULL);
  vmk_Unsigned seed2 = (vmk_Unsigned)(size_t)L;
...
static int math_randomseed (vmk_State *L) {
  ...
  if (vmk_isnone(L, 1)) {
    ...
    vmk_Integer n1 = vmkL_checkinteger(L, 1);
    vmk_Integer n2 = vmkL_optinteger(L, 2, 0);
