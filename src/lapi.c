...
*/
l_sinline StkId index2stack (vmk_State *L, int idx) {
  CallInfo *ci = L->ci;
  if (idx > 0) {
    StkId o = ci->func.p + idx;
...
*/
l_sinline void reverse (vmk_State *L, StkId from, StkId to) {
  for (; from < to; from++, to--) {
    TValue temp;
...
VMK_API vmk_Unsigned vmk_rawlen (vmk_State *L, int idx) {
  const TValue *o = index2value(L, idx);
  switch (ttypetag(o)) {
    case VMK_VSHRSTR: return tsvalue(o)->shrlen;
    case VMK_VLNGSTR: return tsvalue(o)->u.lnglen;
    case VMK_VUSERDATA: return uvalue(o)->len;
...
l_sinline int auxgetstr (vmk_State *L, const TValue *t, const char *k) {
  const TValue *slot;
  TString *str = vmkS_new(L, k);
  if (vmkV_fastget(L, t, str, slot, vmkH_getstr)) {
    ...
    api_incr_top(L);
    vmkV_finishget(L, t, s2v(L->top.p - 1), L->top.p - 1, slot);
  }
  vmk_unlock(L);
...
*/
#define getGtable(L)  \
	(&hvalue(&G(L)->l_registry)->array[VMK_RIDX_GLOBALS - 1])
...
VMK_API int vmk_gettable (vmk_State *L, int idx) {
  ...
  vmk_lock(L);
  t = index2value(L, idx);
  if (vmkV_fastget(L, t, s2v(L->top.p - 1), slot, vmkH_get)) {
...
VMK_API int vmk_geti (vmk_State *L, int idx, vmk_Integer n) {
  ...
  vmk_lock(L);
  t = index2value(L, idx);
  if (vmkV_fastgeti(L, t, n, slot)) {
    setobj2s(L, L->top.p, slot);
  }
  else {
    TValue aux;
    setivalue(&aux, n);
    vmkV_finishget(L, t, &aux, L->top.p, slot);
  }
  api_incr_top(L);
  vmk_unlock(L);
  return ttype(s2v(L->top.p - 1));
...
l_sinline int finishrawget (vmk_State *L, const TValue *val) {
...
static Table *gettable (vmk_State *L, int idx) {
...
VMK_API int vmk_rawgeti (vmk_State *L, int idx, vmk_Integer n) {
  ...
  vmk_lock(L);
  t = gettable(L, idx);
  return finishrawget(L, vmkH_getint(t, n));
}
...
VMK_API int vmk_rawgetp (vmk_State *L, int idx, const void *p) {
  ...
  return finishrawget(L, vmkH_get(t, &k));
}
...
static void auxsetstr (vmk_State *L, const TValue *t, const char *k) {
  ...
  api_checknelems(L, 1);
  if (vmkV_fastget(L, t, str, slot, vmkH_getstr)) {
    vmkV_finishfastset(L, t, slot, s2v(L->top.p - 1));
    L->top.p--;  /* pop value */
  }
  else {
    setsvalue2s(L, L->top.p, str);  /* push 'str' (to make it a TValue) */
    api_incr_top(L);
    vmkV_finishset(L, t, s2v(L->top.p - 1), s2v(L->top.p - 2), slot);
...
VMK_API void vmk_settable (vmk_State *L, int idx) {
  ...
  api_checknelems(L, 2);
  t = index2value(L, idx);
  if (vmkV_fastget(L, t, s2v(L->top.p - 2), slot, vmkH_get)) {
    vmkV_finishfastset(L, t, slot, s2v(L->top.p - 1));
  }
  else
    vmkV_finishset(L, t, s2v(L->top.p - 2), s2v(L->top.p - 1), slot);
...
VMK_API void vmk_seti (vmk_State *L, int idx, vmk_Integer n) {
  ...
  if (vmkV_fastgeti(L, t, n, slot)) {
    ...
    TValue aux;
    setivalue(&aux, n);
    vmkV_finishset(L, t, &aux, s2v(L->top.p - 1), slot);
  }
  L->top.p--;  /* pop value */
  vmk_unlock(L);
}
...
VMK_API int vmk_dump (vmk_State *L, vmk_Writer writer, void *data, int strip) {
  ...
  o = s2v(L->top.p - 1);
  if (isLfunction(o))
    status = vmkU_dump(L, getproto(o), writer, data, strip);
  else
    status = 1;
  vmk_unlock(L);
  return status;
...
VMK_API int vmk_gc (vmk_State *L, int what, ...) {
  ...
  switch (what) {
    ...
    case VMK_GCSTEP: {
      ...
      if (data == 0) {
        vmkE_setdebt(g, 0);  /* do a basic step */
        vmkC_step(L);
      }
    ...
    case VMK_GCGEN: {
      ...
      res = isdecGCmodegen(g) ? VMK_GCGEN : VMK_GCINC;
      if (minormul != 0)
        g->genminormul = minormul;
      if (majormul != 0)
        setgcparam(g->genmajormul, majormul);
      vmkC_changemode(L, KGC_GEN);
      break;
    }
    case VMK_GCINC: {
...
VMK_API void *vmk_newuserdatauv (vmk_State *L, size_t size, int nuvalue) {
  ...
  u = vmkS_newudata(L, size, nuvalue);
