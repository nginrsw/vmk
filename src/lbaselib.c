...
static const char *b_str2int (const char *s, int base, vmk_Integer *pn) {
  vmk_Unsigned n = 0;
  int neg = 0;
...
static int vmkB_rawlen (vmk_State *L) {
  ...
  vmkL_argexpected(L, t == VMK_TTABLE || t == VMK_TSTRING, 1,
                      "table or string");
  vmk_pushinteger(L, vmk_rawlen(L, 1));
  return 1;
}
...
static int vmkB_collectgarbage (vmk_State *L) {
  ...
  static const int optsnum[] = {VMK_GCSTOP, VMK_GCRESTART, VMK_GCCOLLECT,
    VMK_GCCOUNT, VMK_GCSTEP, VMK_GCSETPAUSE, VMK_GCSETSTEPMUL,
    VMK_GCISRUNNING, VMK_GCGEN, VMK_GCINC};
  ...
  switch (o) {
    ...
    case VMK_GCSTEP: {
      int step = (int)vmkL_optinteger(L, 2, 0);
      int res = vmk_gc(L, o, step);
      checkvalres(res);
    ...
    case VMK_GCSETSTEPMUL: {
      int p = (int)vmkL_optinteger(L, 2, 0);
      int previous = vmk_gc(L, o, p);
      checkvalres(previous);
      vmk_pushinteger(L, previous);
      return 1;
    }
    ...
    case VMK_GCGEN: {
      int minormul = (int)vmkL_optinteger(L, 2, 0);
      int majormul = (int)vmkL_optinteger(L, 3, 0);
      return pushmode(L, vmk_gc(L, o, minormul, majormul));
    }
    case VMK_GCINC: {
      int pause = (int)vmkL_optinteger(L, 2, 0);
      int stepmul = (int)vmkL_optinteger(L, 3, 0);
      int stepsize = (int)vmkL_optinteger(L, 4, 0);
      return pushmode(L, vmk_gc(L, o, pause, stepmul, stepsize));
    }
    default: {
      int res = vmk_gc(L, o);
      checkvalres(res);
...
static int vmkB_loadfile (vmk_State *L) {
  ...
  const char *mode = vmkL_optstring(L, 2, NULL);
...
static int vmkB_load (vmk_State *L) {
  ...
  size_t l;
  const char *s = vmk_tolstring(L, 1, &l);
  const char *mode = vmkL_optstring(L, 3, "bt");
