...
#define VMKI_TRY(L,c,a) \
	try { a } catch(...) { if ((c)->status == 0) (c)->status = -1; }
#define vmki_jmpbuf		int  /* dummy variable */

#elif defined(VMK_USE_POSIX)				/* }{ */

/* in POSIX, try _longjmp/_setjmp (more efficient) */
#define VMKI_THROW(L,c)		_longjmp((c)->b, 1)
#define VMKI_TRY(L,c,a)		if (_setjmp((c)->b) == 0) { a }
...
#define VMKI_TRY(L,c,a)		if (setjmp((c)->b) == 0) { a }
#define vmki_jmpbuf		jmp_buf
...
*/
static void correctstack (vmk_State *L) {
  CallInfo *ci;
  UpVal *up;
...
#define ERRORSTACKSIZE	(VMKI_MAXSTACK + 200)
...
int vmkD_reallocstack (vmk_State *L, int newsize, int raiseerror) {
  ...
  vmk_assert(newsize <= VMKI_MAXSTACK || newsize == ERRORSTACKSIZE);
  relstack(L);  /* change pointers to offsets */
  G(L)->gcstopem = 1;  /* stop emergency collection */
  newstack = vmkM_reallocvector(L, L->stack.p, oldsize + EXTRA_STACK,
...
int vmkD_growstack (vmk_State *L, int n, int raiseerror) {
  ...
  if (l_unlikely(size > VMKI_MAXSTACK)) {
  ...
  else if (n < VMKI_MAXSTACK) {  /* avoids arithmetic overflows */
    int newsize = 2 * size;  /* tentative new size */
    int needed = cast_int(L->top.p - L->stack.p) + n;
    if (newsize > VMKI_MAXSTACK)  /* cannot cross the limit */
      newsize = VMKI_MAXSTACK;
    if (newsize < needed)  /* but must respect what was asked for */
      newsize = needed;
    if (l_likely(newsize <= VMKI_MAXSTACK))
...
** it is not, 'max' (limited by VMKI_MAXSTACK) will be smaller than
** stacksize (equal to ERRORSTACKSIZE in this case), and so the stack
** will be reduced to a "regular" size.
*/
void vmkD_shrinkstack (vmk_State *L) {
  int inuse = stackinuse(L);
  int max = (inuse > VMKI_MAXSTACK / 3) ? VMKI_MAXSTACK : inuse * 3;
  /* if thread is currently not handling a stack overflow and its
     size is larger than maximum "reasonable" size, shrink it */
  if (inuse <= VMKI_MAXSTACK && stacksize(L) > max) {
    int nsize = (inuse > VMKI_MAXSTACK / 2) ? VMKI_MAXSTACK : inuse * 2;
...
** stack, below original 'func', so that 'vmkD_precall' can call it. Raise
** an error if there is no '__call' metafield.
*/
static StkId tryfuncTM (vmk_State *L, StkId func) {
  const TValue *tm;
  StkId p;
  checkstackGCp(L, 1, func);  /* space for metamethod */
  tm = vmkT_gettmbyobj(L, s2v(func), TM_CALL);  /* (after previous GC) */
  if (l_unlikely(ttisnil(tm)))
    vmkG_callerror(L, s2v(func));  /* nothing to call */
...
** separated.
*/
l_sinline void moveresults (vmk_State *L, StkId res, int nres, int wanted) {
  StkId firstresult;
  int i;
...
l_sinline CallInfo *prepCallInfo (vmk_State *L, StkId func, int nret,
                                                int mask, StkId top) {
...
*/
l_sinline int precallC (vmk_State *L, StkId func, int nresults,
                                            vmk_CFunction f) {
  int n;  /* number of returns */
  CallInfo *ci;
  checkstackGCp(L, VMK_MINSTACK, func);  /* ensure minimum stack size */
...
VMK_API int vmk_resume (vmk_State *L, vmk_State *from, int nargs,
  ...
  api_checknelems(L, (L->status == VMK_OK) ? nargs + 1 : nargs);
...
int vmkD_protectedparser (vmk_State *L, ZIO *z, const char *name,
  ...
  vmkM_freearray(L, p.dyd.actvar.arr, p.dyd.actvar.size);
  vmkM_freearray(L, p.dyd.gt.arr, p.dyd.gt.size);
  vmkM_freearray(L, p.dyd.label.arr, p.dyd.label.size);
  decnny(L);
  return status;
}
