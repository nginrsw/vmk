...
void vmkF_freeproto (vmk_State *L, Proto *f) {
  vmkM_freearray(L, f->code, f->sizecode);
  vmkM_freearray(L, f->p, f->sizep);
  vmkM_freearray(L, f->k, f->sizek);
  vmkM_freearray(L, f->lineinfo, f->sizelineinfo);
  vmkM_freearray(L, f->abslineinfo, f->sizeabslineinfo);
  vmkM_freearray(L, f->locvars, f->sizelocvars);
  vmkM_freearray(L, f->upvalues, f->sizeupvalues);
  vmkM_free(L, f);
}
